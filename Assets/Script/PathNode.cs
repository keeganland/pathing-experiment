...
public class PathNode : IHeapItem<PathNode> {
    ...
    public int Get_fCost //A* algorithm's methodology is always select node with lowest fCost
    {
    ...
    public void Set_gCost(int n)
    {
        gCost = n;
    }
    public void Set_hCost(int n)
    {
        hCost = n;
    }
    ...
    {
        int compare = Get_fCost.CompareTo(other.Get_fCost);
        if (compare == 0)
        {
            compare = Get_hCost().CompareTo(other.Get_hCost()); //remember, hCost is the tiebreaker
        }
